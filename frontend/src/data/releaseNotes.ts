...
export const releases: Release[] = [
  {
    ...
    items: [
      ...
      {
        tag: 'feature',
        text: '本番環境を Firebase Hosting にデプロイ（https://scenariolab-studio.web.app）',
      },
      {
        tag: 'improvement',
        text: 'パッケージ名を `@scenario-lab/*` に統一（旧 scenario-writing-lab）',
      },
      {
        tag: 'improvement',
        text: 'CI に auto-format ステップ・Hosting デプロイワークフローを追加',
      },
      {
        tag: 'improvement',
        text: '仕様書に Q1–Q5 の明確化セッションを統合（AI プロバイダー、失敗ハンドリング、エクスポート形式、ストレージ、認証方式）',
      },
      {
        tag: 'improvement',
        text: 'speckit エージェントスキル（clarify / plan / tasks / implement 等）を追加',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        tag: 'feature',
        text: 'テンプレートベースの脚本出力（応募用 docx テンプレートに合わせた書式）',
      },
      { tag: 'feature', text: '形式プリセット（投稿先ごとの行数・文字数設定）' },
      {
        tag: 'fix',
        text: 'テンプレート出力時のフォント保持とフィールドマッピングのフォールバックを修正',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        tag: 'feature',
        text: '200字詰原稿用紙準拠の `linesPerPage` 設定（脚本提出書式に合わせて自動整形）',
      },
      { tag: 'feature', text: 'V2 縦書きエディタ（IME・popup ログイン対応強化、2026-05 追加）' },
      {
        tag: 'improvement',
        text: 'プレーンテキスト形式（.txt）のエクスポート・タイトル別ページ・「Save As」ダイアログ対応',
      },
      {
        tag: 'fix',
        text: 'popup ブロック時に signInWithRedirect へフォールバック（ログイン安定化）',
      },
