    ...
    <>
      {pages.map((pageCols, pageIdx) => (
        <div key={pageIdx} style={PAGE_STYLE}>
          ...
          >
            {pageCols.length === 0 ? (
              <div style={{ writingMode: 'vertical-rl', color: '#d1d5db', fontSize: '0.8125rem' }}>
                &nbsp;
              </div>
            ) : (
        ...
        <>
          {characterPages.map((pageCols, pageIdx) => (
            <div key={`char-${pageIdx}`} style={PAGE_STYLE}>
              ...
              >
                {pageIdx === 0 && (
                  <div style={{ ...SECTION_HEADING_STYLE, marginLeft: '1.5rem', marginBottom: 0 }}>
                    登場人物
                  </div>
                )}
                <div
