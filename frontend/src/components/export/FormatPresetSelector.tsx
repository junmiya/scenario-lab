...
export function FormatPresetSelector({
    userId, selectedPresetId, onSelect, onApply, onReset, refreshKey,
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
            ...
            {selectedPresetId && (
                <>
                    <button type="button" onClick={handleApply} style={{ ...BTN_STYLE, color: 'var(--text-primary)', fontWeight: 600 }}>
                        適用
                    </button>
                    <button type="button" onClick={onReset} style={BTN_STYLE}>
                        リセット
                    </button>
                    <button type="button" onClick={() => void handleDelete()} title="プリセットを削除" style={BTN_STYLE}>
                        削除
                    </button>
                </>
            )}
