...
interface CachedData { story: CommentEntry[]; producer: CommentEntry[]; proofreader: CommentEntry[] }
    ...
    try {
        ...
        if (typeof parsed.story === 'string' && parsed.story !== DEFAULT_MSG) {
            return {
                ...
                producer: typeof parsed.market === 'string' && parsed.market !== DEFAULT_MSG
        ...
        return {
            ...
            proofreader: Array.isArray(parsed.proofreader) ? parsed.proofreader : (Array.isArray(parsed.doctor) ? parsed.doctor : []),
        };
    } catch { /* ignore */ }
    return { story: [], producer: [], proofreader: [] };
}

function saveCached(scriptId: string | undefined, story: CommentEntry[], producer: CommentEntry[], proofreader: CommentEntry[]) {
    if (!scriptId) return;
...
export function SynopsisCommentary({ synopsis, scriptId, charsPerColumn = 20, pageCount = 2, children }: SynopsisCommentaryProps): ReactElement {
    ...
    // Doctor panel state
    const [proofreaderHistory, setProofreaderHistory] = useState<CommentEntry[]>(cached.current.proofreader);
    ...
    const generateStory = () => {
        ...
        void callAi(provider, STORY_PROMPT, userText)
            ...
            .finally(() => { if (storyAbortRef.current === gen) setStoryLoading(false); });
    };
    ...
    const generateProducer = () => {
        ...
        void callAi(provider, MARKET_PROMPT, userText)
            ...
            .finally(() => { if (producerAbortRef.current === gen) setProducerLoading(false); });
    };
    ...
    const generateProofreader = () => {
        ...
        void callAi(provider, PROOFREADER_PROMPT, userText)
            ...
            .catch((err) => {
                if (proofreaderAbortRef.current === gen) {
                    const msg = `エラー: ${err instanceof Error ? err.message : String(err)}`;
                    setProofreaderHistory((prev) => [{ text: msg, provider, timestamp: Date.now() }, ...prev]);
                    setProofreaderIndex(0);
                }
            })
            .finally(() => { if (proofreaderAbortRef.current === gen) setProofreaderLoading(false); });
    };
    ...
    useEffect(() => {
        ...
        return () => { if (producerTimerRef.current) clearTimeout(producerTimerRef.current); };
    ...
    useEffect(() => {
        ...
        return () => { if (proofreaderTimerRef.current) clearTimeout(proofreaderTimerRef.current); };
    }, [synopsis, proofreaderAuto]);

    const controlsStyle: React.CSSProperties = { fontSize: '0.6875rem', padding: '0.125rem 0.25rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-bg-primary)', color: 'var(--text-primary)' };

    const panelHeader = (
        icon: ReactNode, label: string, loading: boolean, loadingColor: string,
        auto: boolean, setAuto: (v: (prev: boolean) => boolean) => void,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem' }}>
                ...
                {historyLen > 0 && !loading && (
                    <span style={{
                        ...
                        <button type="button" disabled={historyIdx >= historyLen - 1} onClick={() => setHistoryIdx(historyIdx + 1)} style={{
                            fontSize: '0.6875rem', padding: '0 0.125rem', border: 'none', background: 'none',
                            cursor: historyIdx < historyLen - 1 ? 'pointer' : 'default',
                            color: historyIdx < historyLen - 1 ? 'var(--text-primary)' : 'var(--text-secondary)',
                            opacity: historyIdx < historyLen - 1 ? 1 : 0.3,
                        }}>&#9664;</button>
                        <span style={{ color: 'var(--text-primary)', minWidth: '2em', textAlign: 'center' }}>{historyIdx + 1}/{historyLen}</span>
                        <button type="button" disabled={historyIdx <= 0} onClick={() => setHistoryIdx(historyIdx - 1)} style={{
                            fontSize: '0.6875rem', padding: '0 0.125rem', border: 'none', background: 'none',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', flexShrink: 0 }}>
                <span style={{ fontSize: '0.625rem', color: 'var(--text-secondary)', whiteSpace: 'nowrap' }}>
                    ...
                    <button type="button" onClick={() => setShowDel((v) => !v)} style={{
                        fontSize: '0.625rem', fontWeight: 600, borderRadius: '2px', padding: '0 2px', cursor: 'pointer',
                        border: 'none',
                ...
                <button type="button" onClick={() => setAuto((v) => !v)} style={{
                    fontSize: '0.625rem', padding: '0.125rem 0.375rem', borderRadius: 'var(--radius-sm)',
                ...
                <select value={provider} onChange={(e) => setProvider(e.target.value as AiProvider)} style={{ ...controlsStyle, fontSize: '0.625rem' }}>
                    <option value="gemini">Gemini</option>
                    <option value="claude">Claude</option>
                </select>
                <button type="button" onClick={onGenerate} disabled={loading || !hasEnoughText} style={{ fontSize: '0.625rem', padding: '0.125rem 0.375rem' }}>
                    {loading ? '...' : 'AI分析'}
                </button>
            </div>
        </div>
    ...
    const panelBody = (comment: string, loading: boolean, showDel: boolean) => {
        ...
        return (
            <div>
                <div className="vertical-editor-container" style={{ borderTop: 'none', borderRadius: isDefault ? '0 0 var(--radius-lg) var(--radius-lg)' : '0' }}>
        ...
        <>
            ...
            <div style={{ marginBottom: '0.75rem' }}>
                {panelHeader(
                    ...
                    'ストーリー分析',
                    storyLoading, '#6366f1',
                    storyAuto, setStoryAuto,
                    storyProvider, setStoryProvider,
                    generateStory,
            ...
            <div style={{ marginTop: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', gap: 0 }}>
                    {(['producer', 'proofreader'] as SubRole[]).map((r) => {
                        ...
                        return (
                            <button key={r} type="button" onClick={() => setSubRole(r)} style={{
                                ...
                                {r === 'producer'
                                    ? <Lightbulb size={11} style={{ color: active ? iconColor : 'var(--text-secondary)' }} />
                                    : <SpellCheck size={11} style={{ color: active ? iconColor : 'var(--text-secondary)' }} />}
                                {SUB_ROLE_LABELS[r]}
                            </button>
                        );
                    })}
                ...
                {subRole === 'producer' ? (
                    <>
                        {panelHeader(
                            ...
                            producerLoading, SUB_ROLE_ICONS.producer.color,
                            producerAuto, setProducerAuto,
                            producerProvider, setProducerProvider,
                            generateProducer,
                    ...
                    <>
                        {panelHeader(
                            ...
                            proofreaderLoading, SUB_ROLE_ICONS.proofreader.color,
                            proofreaderAuto, setProofreaderAuto,
                            proofreaderProvider, setProofreaderProvider,
                            generateProofreader,
