        ...
        <Layout headerTitle="コンテスト">
            <div style={{ maxWidth: '900px', margin: '0 auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
                    <div>
                        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: 'var(--text-primary)', margin: 0 }}>コンテスト</h1>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', marginTop: '0.25rem' }}>脚本コンテストの一覧です。</p>
                    </div>
                    {canCreate && (
                        <button
                            ...
                            style={{
                                ...
                                color: 'var(--color-surface)', borderRadius: 'var(--radius-lg)',
                ...
                {loading && <div style={{ textAlign: 'center', padding: '4rem 0', color: 'var(--text-secondary)' }}>読み込み中...</div>}

                {!loading && contests.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '4rem 2rem', border: '2px dashed var(--border)', borderRadius: 'var(--radius-xl)', backgroundColor: 'var(--color-surface)' }}>
                        <Trophy size={48} style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }} />
                        <h3 style={{ fontSize: '1.125rem', fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 0.5rem' }}>コンテストはまだありません</h3>
                    </div>
                )}
                                ...
                                >
                                    <div>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                            <h3 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-primary)', margin: 0 }}>{c.name}</h3>
                                            <span style={{ fontSize: '0.75rem', padding: '0.125rem 0.5rem', borderRadius: 'var(--radius-sm)', backgroundColor: si.bg, color: si.color, fontWeight: 500 }}>
                                                {si.label}
                                            </span>
                                        </div>
                                        {c.description && <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', margin: '0.25rem 0 0' }}>{c.description}</p>}
                                    </div>
