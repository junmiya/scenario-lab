...
export function CorrectionPage(): ReactElement {
  ...
  const handleTextSelect = useCallback(
    (sel: { startOffset: number; endOffset: number; text: string }) => {
      if (!isTeacher) return;
          ...
          <div>
            ...
            >
              ...
              {corrections.length > 0 &&
                ` ・ 添削${corrections.length}件 (未確認${pendingCount} / 承認${acceptedCount})`}
            </p>
          </div>
        </div>
          ...
          <div style={{ flex: '1 1 65%', minWidth: 0 }}>
            {currentText ? (
              <CorrectionEditor
                ...
                lineCount={Math.max(
                  10,
                  Math.ceil(currentText.length / (snap.settings?.lineLength ?? 20)),
                )}
                onTextSelect={isTeacher ? handleTextSelect : undefined}
                onCorrectionClick={(id) =>
                  setActiveCorrectionId(id === activeCorrectionId ? null : id)
                }
              />
            ) : (
