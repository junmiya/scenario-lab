...
export async function updateScript(
    scriptId: string,
    data: Partial<Pick<FirestoreScript, 'title' | 'authorName' | 'synopsis' | 'content' | 'characterText' | 'characters' | 'settings' | 'synopsisSettings' | 'characterSettings' | 'contentCommentary' | 'synopsisCommentary' | 'discussionMessages'>>,
): Promise<void> {
    const docRef = doc(db, 'scripts', scriptId);
...
export async function saveVersion(scriptId: string, data: Omit<ScriptVersion, 'id' | 'savedAt'>): Promise<void> {
...
export async function getVersion(scriptId: string, versionId: string): Promise<ScriptVersion | null> {
...
export async function updateFeatureFlags(flags: Partial<FeatureFlags>): Promise<void> {
    await setDoc(doc(db, 'config', 'features'), { ...flags, updatedAt: serverTimestamp() }, { merge: true });
}
...
export async function createGroup(
    createdBy: string, name: string, description: string,
...
export async function addGroupMember(groupId: string, uid: string, role: GroupMember['role'], displayName?: string, email?: string): Promise<void> {
...
export async function createSubmission(groupId: string, scriptId: string, submittedBy: string, script: FirestoreScript): Promise<string> {
...
export async function getSubmission(groupId: string, submissionId: string): Promise<GroupSubmission | null> {
...
export async function updateSubmissionStatus(groupId: string, submissionId: string, status: GroupSubmission['status'], reviewedBy?: string): Promise<void> {
...
export async function createComment(data: Omit<FirestoreComment, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
...
export async function listComments(submissionId: string, groupId: string): Promise<FirestoreComment[]> {
    const q = query(
...
export async function listCorrections(groupId: string, submissionId: string): Promise<Correction[]> {
    const q = query(
...
export async function updateCorrectionStatus(
    groupId: string, submissionId: string, correctionId: string,
...
export async function createContest(data: Omit<Contest, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
...
export async function updateContest(contestId: string, data: Partial<Omit<Contest, 'id'>>): Promise<void> {
    await updateDoc(doc(db, 'contests', contestId), { ...data, updatedAt: serverTimestamp() });
}

export async function createContestEntry(contestId: string, scriptId: string, submittedBy: string, script: FirestoreScript): Promise<string> {
...
export async function getContestEntry(contestId: string, entryId: string): Promise<ContestEntry | null> {
...
export async function saveEvaluation(contestId: string, entryId: string, evaluation: Evaluation): Promise<void> {
    await setDoc(
...
export async function getEvaluation(contestId: string, entryId: string, evaluatorId: string): Promise<Evaluation | null> {
    const snap = await getDoc(doc(db, 'contests', contestId, 'entries', entryId, 'evaluations', evaluatorId));
...
export async function listEvaluations(contestId: string, entryId: string): Promise<Evaluation[]> {
    const snap = await getDocs(collection(db, 'contests', contestId, 'entries', entryId, 'evaluations'));
